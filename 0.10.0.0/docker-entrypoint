...
if [[ -z "$CONNECT_BOOTSTRAP_SERVERS" ]]; then
    error "EMPTY ENV 'CONNECT_BOOTSTRAP_SERVERS'"
    exit 1
fi

if [[ -z "$CONNECT_REST_ADVERTISED_HOST_NAME" ]]; then
    warn "EMPTY ENV 'CONNECT_REST_ADVERTISED_HOST_NAME'"
    unset $CONNECT_REST_ADVERTISED_HOST_NAME
fi

if [[ -z "$CONNECT_REST_ADVERTISED_PORT" ]]; then
    warn "EMPTY ENV 'CONNECT_REST_ADVERTISED_PORT'"
    unset $CONNECT_REST_ADVERTISED_PORT
fi
if [[ -z "$CONNECT_GROUP_ID" ]]; then
    warn "EMPTY ENV 'CONNECT_GROUP_ID'. USE DEFAULT VALUE"
    unset $CONNECT_GROUP_ID
fi
